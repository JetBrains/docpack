...
DocsPlugin.createExamplesCompilerConfig = function(cfg) {
  var config = clone(cfg);

  if (config.plugins) {
    config.plugins = cfg.plugins.filter(function (plugin) {
      return !(plugin instanceof DocsPlugin);
    });
  }

  if (config.module && config.module.loaders) {
    config.module.loaders = config.module.loaders.filter(function (loaderConfig) {
      return loaderConfig.loader.indexOf(docsLoaderPath) === -1;
    });
  }
...
DocsPlugin.createExamplesCompiler = function(config) {
  var compiler = webpack(config);
  compiler.outputFyleSystem = new MemoryFileSystem();
  return compiler;
};
