...
TemplateCompiler.prototype.run = function() {
  ...
  return ChildCompiler.prototype.run.call(this)
    .then(function (compilation) {
      ...
      var isCached = templateRequest in cache && cache[templateRequest].hash == hash;

      if (isCached) {
        return Promise.resolve(cache[templateRequest].template);
      }
      else {
        return compiler.evaluateCompilationResult(source).then(function(func) {
          cache[templateRequest] = {
            hash: hash,
            template: func
          };
