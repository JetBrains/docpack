...
function ExamplesCompiler(compilation, options, examples, resourcePath) {
  ...
  sharedDataLoader.pluginInCompiler(this, examples);
}
 ...
 * @returns {string}
 */
ExamplesCompiler.prototype.createEntryRequest = function(file, key) {
  var parentCompilerConfig = this.compiler.options;
  var resourcePath = this.resourcePath;
  return ExamplesCompiler.createEntryRequest(file, key, parentCompilerConfig, resourcePath);
};
...
ExamplesCompiler.prototype.createEntries = function (examples, context) {
  var createEntryRequest = this.createEntryRequest;
  var entries = [];

  examples.forEach(function (example, exampleIndex) {
    example.files.forEach(function (file, fileIndex) {
      var key = format('%s.files.%s', exampleIndex.toString(), fileIndex.toString());
      var request = createEntryRequest(file, key);
