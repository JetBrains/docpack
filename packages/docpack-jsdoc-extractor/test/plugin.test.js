...
var PluginRewired = require('rewire')('../lib/plugin');
...
describe('docpack-jsdoc-extractor', () => {
  ...
  describe('apply()', () => {
    ...
    it('should work', (done) => {
      var spiedExtractor = sinon.spy(extractor);
      PluginRewired.__set__('extractor', spiedExtractor);

      Compiler({
        plugins: [
          Docpack()
            .use(Docpack.HOOKS.BEFORE_EXTRACT, (s, done) => done(null, sources))
        ...
        .then(compilation => {
          ...
          var call = spiedExtractor.firstCall;

          // Extractor context
          var thisValue = call.thisValue;
